...
export function authTest(): Promise<{resCode: string, msg: string, res: boolean, data?: {username: string}, redirect: string}> {
    ...
    return new Promise((resolve, reject): void => {
        serverTest().then(data => {
            ...
            fetch(`${serverUrl}/auth`, {
                ...
                if (!json || !json.res) {
                    Token.remove();
                    resolve({resCode: "UNAUTH", msg: "Unauthorized or token expired", res: false, redirect: "/login?redirected" });
                } else {
                    resolve({resCode: "AUTH_OK", msg: "Authorized and server is running", res: true, data: {username: json.res}, redirect: "/" })
                }
            })
        }).catch(error => {
                resolve({resCode: "AUTH_ERROR", msg: "Error while trying to authenticate", res: false, redirect: "/status?redirected"});
        })
    })
...
export function serverTest(): Promise<{resCode: string, msg: string, res: boolean, redirect: string}> {
    return new Promise((resolve, reject): void => {
        fetch(`${serverUrl}/health`).then(response => {
            if (!response.ok) {
                resolve({resCode: "SERVER_DOWN", msg: "Server is down", res: false, redirect: "/status?redirected"});
            } else {
                resolve({resCode: "SERVER_OK", msg: "Server is running", res: true, redirect: "/"});
            }
        }).catch(error => {
            resolve({resCode: "SERVER_DOWN", msg: "Server is down", res: false, redirect: "/status?redirected"});
        })
